...
import new_eDoe.comparators.DescricaoComparator;
import new_eDoe.comparators.IdComparator;
import new_eDoe.comparators.QuantidadeComparator;
...
public class GestorItem implements Serializable {
     ...
     */
    public int adicionaItemParaDoacao(Usuario doador, String descricao, int quantidade, int idItem) {
        ...
        return doador.adicionaItemParaDoacao(descricao, quantidade,false, idItem);
    }
    ...
    //	 * @param tags       Novas tags que caracterizam o item
     * @return String do item com as informações atualizadas
     */
    public String atualizaItemParaDoacao(Usuario user, int idItem, int quantidade) {
        Validador.verificadorAtualizaItemParaDocao(user, idItem, quantidade);
        String saida = user.atualizaItem(idItem, quantidade);
     ...
     */
    public int adicionaItemNecessario(Usuario receptor, String descricao, int quantidade, int idItem) {
        Validador.validadorAdicionaItem(descricao, quantidade);
        String descrMin = descricao.toLowerCase().trim();
        return receptor.adicionaItemNecessario(descrMin, quantidade, true, idItem);
    }
     ...
     * @return String do item com as informações atualizadas
     */
    public String atualizaNecessario(Usuario user, int idItem, int novaQuantidade) {
        Validador.verificadorAtualizaItemNecessario(user, idItem);
        String saida = user.atualizaItem(idItem, novaQuantidade);
        atualizaDescritores(user, idItem, "atualizaItem");
        return saida;
    }
    ...
    public String match(Usuario user, int idItemNecessario, Map<String, Usuario> todosUsuarios) {
        ...
        Collections.sort(arrayItensRequeridos);
        return makeListaItens(arrayItensRequeridos, "fichaTecnica");
    }
    /**
